class RailRoad
  ...
  def create_wagon
    ...
    if wagon_type == :cargo
      wagon_volume = @interface.ask_wagon_volume
      @wagons << CargoWagon.new(wagon_number, wagon_volume)
      @interface.successful_creating
    elsif wagon_type == :passenger
      wagon_seats = @interface.ask_wagon_seats
      @wagons << PassengerWagon.new(wagon_number, wagon_seats)
      @interface.successful_creating
    end
    rescue RuntimeError => e
      @interface.creating_error(e)
  ...
  def wagon_data_for_connection
    return wagons_list if wagons.empty?
    return trains_list if trains.empty?
    wagons_list
    wagon = request_wagon
    trains_list
    train = request_train
    return if wagon.nil? || train.nil?
    data_for_connection = [train, wagon]
  end

  def wagon_connect_to
    data_for_connection = wagon_data_for_connection
    return @interface.error if data_for_connection.nil?
    train = data_for_connection[0]
    wagon = data_for_connection[1]
