class Interface
  ...
  def ask_train_number
    loop do
      ...
      break(train_number) unless train_number.empty?
    end
  end
  ...
  def ask_train_type
    loop do
      ...
      break(train_type) if %i[cargo passenger].include?(train_type)
    end
  end
  ...
  def ask_train_speed_mode
    loop do
      ...
      break(speed_mode) if [1, 2].include?(speed_mode)
    end
  end
  ...
  def ask_train_speed_value
    loop do
      ...
      break(speed_value) unless speed_value.zero?
    end
  end
  ...
  def ask_train_move_mode
    loop do
      ...
      move_mode = gets.to_i
      break(move_mode) if [1, 2].include?(move_mode)
    end
  end
  ...
  def ask_route_name
    loop do
      ...
      break(route_name) unless route_name.empty?
    end
  end
  ...
  def ask_station_departure
    loop do
      ...
      break(station_departure_name) unless station_departure_name.empty?
    end
  end
  ...
  def ask_station_arrival
    loop do
      ...
      break(station_arrival_name) unless station_arrival_name.empty?
    end
  end
  ...
  def ask_wagon_type
    loop do
      ...
      break(wagon_type) if %i[cargo passenger].include?(wagon_type)
    end
  end
  ...
  def wagon_data(wagon)
    ...
    attributes.each do |attribute, value|
      print "#{attribute}: #{value} #{wagon.type == :passenger ? 'мест' : 'кубометров'}, "
    end
    puts "\n--------------------------------------------------------------------------\n"
  end
